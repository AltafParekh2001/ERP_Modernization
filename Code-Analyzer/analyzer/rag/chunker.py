...
class CodeChunker:
    ...
    def _create_module_chunk(self, module: Module) -> Optional[CodeChunk]:
        ...
        is_go = module.file_path.endswith('.go')
        
        # Package name for Go
        if is_go and module.package:
            parts.append(f"Package: {module.package}")
        ...
        if module.variables:
            parts.append(f"Variables: {len(module.variables)}")
        if module.constants:
            parts.append(f"Constants: {len(module.constants)}")
        
        content = "\n".join(parts)
        
        if len(content) < 20:  # Skip empty modules (threshold lowered from 50)
            return None
    ...
    def _create_class_overview_chunk(self, cls: Class, module: Module) -> Optional[CodeChunk]:
        ...
        if is_go:
            # Format as Go struct
            if cls.is_abstract:
                parts.append(f"type {cls.name} interface {{")
            else:
        ...
        if cls.methods:
            ...
            for method in cls.methods:
                sig = self._get_method_signature(method, is_go=is_go)
                parts.append(f"    {sig}")
    ...
    def _get_method_signature(
        self, method: Method, include_types: bool = False, is_go: bool = False
    ) -> str:
        ...
        if is_go:
            ...
            return f"func {method.name}({', '.join(params)}){ret_type}"
        else:
